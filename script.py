...
def save_company_to_wordpress(index, company_data, wp_headers):
    ...
    post_data = {
        ...
        "meta": {
            "_company_name": sanitize_text(text=company_name),
...
def save_article_to_wordpress(index, job_data, company_id, wp_headers, live_jobs):
    ...
    post_data = {
        ...
        "job_listing_type": [job_type_id] if (job_type_id := get_or_create_term(job_type, "job_type", WP_JOB_TYPE_URL, wp_headers)) else [],
        "job_listing_region": [job_region_id] if (job_region_id := get_or_create_term(location, "job_region", WP_JOB_REGION_URL, wp_headers)) else []
    }
    
    # Add to live jobs for scraped_jobs.json
    live_jobs.append({
        "title": job_title,
...
def crawl(auth_headers, processed_ids):
    ...
    for i in range(start_page, 15):
        ...
        try:
            ...
            logger.info(f'Found {len(urls)} job URLs on page: {search_url}')
            ...
            for index, job_url in enumerate(urls):
                ...
                try:
                    ...
                    job_post_id, job_post_url = save_article_to_wordpress(index, job_dict, company_id, auth_headers, live_jobs)
